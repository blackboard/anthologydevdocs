...
### Prerequisites
- You must [register a developer account and application](../Getting%20Started/registry.md) in the Developer Portal
- You must
  [register your application](../Getting%20Started/registry.md) in Learn
...
1. Use the Application Key and Secret to obtain an **OAuth 2.0 access token**, as described in the [Basic Authentication](../Getting%20Started/basic-authentication.md) document.

**OR**

1. Use the authorizationcode endpoint to log into Learn with a given user's credentials, then obtain an **OAuth 2.0 access token**, using the application key/secret, that only allows access to the system based on that particular user's entitlements as described in the [Three-Legged OAuth](../Getting%20Started/3lo.md) document.
